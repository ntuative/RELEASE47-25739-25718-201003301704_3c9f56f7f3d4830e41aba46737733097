package com.sulake.habbo.friendlist.domain
{
   ...
   public class FriendCategories
   {
      ...
      private var var_382:IFriendCategoriesDeps;
      ...
      private function updateCategories(param1:Array) : void
      {
         ...
         var _loc5_:* = null;
         flushReceivedStatus();
         findCategory(FriendCategory.const_465).received = true;
         findCategory(FriendCategory.const_807).received = true;
      ...
      public function onFriendListUpdate(param1:IMessageEvent) : void
      {
         ...
         for each(_loc4_ in _loc2_.updatedFriends)
         {
            ...
            var_382.messenger.setFollowingAllowed(_loc4_.id,_loc4_.followingAllowed && _loc4_.online);
            _loc6_ = isFriendOnline(_loc4_.id);
            if(_loc6_ && !_loc4_.online)
            {
               var_382.messenger.setOnlineStatus(_loc4_.id,_loc4_.online);
               var_382.notifications.addOfflineNotification(_loc4_.name,_loc4_.realName);
            }
            if(!_loc6_ && _loc4_.online)
            {
               var_382.messenger.setOnlineStatus(_loc4_.id,_loc4_.online);
               var_382.view.setNewMessageArrived();
               var_382.notifications.addOnlineNotification(_loc4_.name,_loc4_.figure,_loc4_.realName);
            }
