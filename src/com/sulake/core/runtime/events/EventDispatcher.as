package com.sulake.core.runtime.events
{
   ...
   public class EventDispatcher implements IEventDispatcher, IDisposable
   {
      ...
      private static const const_1027:uint = 1;
      ...
      public function dispatchEvent(param1:Event) : Boolean
      {
         if(!_disposed)
         {
            ...
            if(_result == const_739)
            {
               if(var_631 != null)
               {
                  Core.crash("Error catched in EventDispatcher: undefined",var_631.errorID,var_631);
               }
               else
               {
                  Core.crash("Error catched in EventDispatcher. No error data available!",0,var_631);
               }
            }
      ...
      private function eventProcessor(param1:Event) : void
      {
         ...
         var array:Array = var_243[event.type];
         if(array)
         {
            callbacks = [];
         ...
         _result = !!event.isDefaultPrevented() ? uint(const_1027) : uint(const_484);
      }
      ...
      public function dispose() : void
      {
         ...
         if(!_disposed)
         {
            for(_loc1_ in var_243)
            {
               _loc2_ = var_243[_loc1_] as Array;
               for each(_loc3_ in _loc2_)
               {
                  _loc3_.callback = null;
               }
               delete var_243[_loc1_];
            }
            var_243 = null;
