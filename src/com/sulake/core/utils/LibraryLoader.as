package com.sulake.core.utils
{
   ...
   public class LibraryLoader extends EventDispatcher implements IDisposable
   {
      ...
      protected var var_938:int;
      
      protected var var_2279:String = "";
      
      protected var var_370:int;
      ...
      {
         var_2278 = param1;
         dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_ERROR,var_370));
      }
      ...
      protected function prepareLibrary() : void
      {
         ...
         var_344 = this.getDefinition(_name) as Class;
         if(var_344 == null)
         {
         ...
         try
         {
            xmlClass = var_344.manifest as Class;
            if(xmlClass == null)
            {
               return;
            }
      ...
      protected function debug(param1:String) : void
      {
         ...
         if(_debug)
         {
            dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_DEBUG,var_370));
         }
      }
      ...
      protected function loadEventHandler(param1:Event) : void
      {
         switch(param1.type)
         {
            ...
            case HTTPStatusEvent.HTTP_STATUS:
               ...
               debug("Load event STATUS " + var_370 + " for file \"" + var_18.contentLoaderInfo.url + "\"");
               break;
            case Event.UNLOAD:
               debug("Load event UNLOAD for file \"" + var_18.contentLoaderInfo.url + "\"");
               dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_UNLOAD,var_370));
               break;
            case ProgressEvent.PROGRESS:
               debug("Load event PROGRESS for file \"" + var_18.contentLoaderInfo.url + "\"");
               dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_PROGRESS,var_370));
               break;
            case IOErrorEvent.IO_ERROR:
               failure("IO Error, send or load operation failed for file \"" + var_18.contentLoaderInfo.url + "\"");
               if(!handleHttpStatus(var_370))
               {
         ...
         if(var_442 == LibraryLoader.STATE_READY)
         {
            ...
            prepareLibrary();
            removeEventListeners();
            dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_COMPLETE,var_370));
         }
      }
      ...
      protected function handleHttpStatus(param1:int) : Boolean
      {
         if(param1 >= 400)
         {
            if(var_938 > 0)
            {
               var_18.close();
               var_18.unload();
               addRequestCounterToUrlRequest(var_325,const_1397 - var_938);
